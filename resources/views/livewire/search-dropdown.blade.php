            ...
            @if (strlen($search) > 2)
                <ul
                    ...
                    @forelse ($search_results as $result)
                        <li>
                            <a href="{{ array_key_exists('trackViewUrl', $result) ? $result['trackViewUrl'] : '#' }}"
                               class="flex items-center px-4 py-4 hover:bg-gray-200 transition ease-in-out duration-150">
                                ...
                                <div class="ml-4 leading-tight">
                                    <div class="font-semibold">
                                        {{ array_key_exists('trackName', $result) ? $result['trackName'] : 'Untitled' }}
                                    </div>
                                    <div class="text-gray-600">
                                        {{ array_key_exists('artistName', $result) ? $result['artistName'] : 'No Artist' }}
                                    </div>
                                </div>
                            </a>
                        </li>
